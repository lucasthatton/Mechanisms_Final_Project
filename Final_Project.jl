...
# ╔═╡ 31ff7734-5ed7-44fb-a4fe-f43546fa2809
using PlutoUI

# ╔═╡ 3be3ac8d-b99c-499e-b637-dd47390d21be
using Plots

# ╔═╡ d246834a-d179-11ec-22e8-d35b2843840a
# SymVec stores the symbols of a vector 
begin
	struct SymVec
		R::Symbol
		θ::Symbol
	end
	SymVec(V::Symbol) = SymVec(splitVec(V)...)
	SymVec(R::String, θ::String) = SymVec(Symbol(R), Symbol(θ))
	SymVec(V::String) = SymVec(Symbol(V))

	function splitVec(sym)
		strSym = String(sym)
		num = strSym[2:end]
		pair = (Symbol("r", num), Symbol("θ", num))
		return pair
	end

	function xComp(V::SymVec)
		str = String(V.R)*"*cos("*String(V.θ)*")"
		Meta.parse(str)
	end

	function yComp(V::SymVec)
		str = String(V.R)*"*sin("*String(V.θ)*")"
		Meta.parse(str)
	end
	...
	macro vectors(Vs...)
		...
		all_code.args = reduce(vcat, _vectors(V) for V in Vs)

		return all_code
	end

	function _vectors(V)
		:($(esc(V)) = briVec(SymVec($(QuoteNode(V)))))
	end

	function addExp(ex1::Expr, ex2::Expr)
		:(+($(ex1), $(ex2)))
	end

	function subExp(ex1::Expr, ex2::Expr)
		:(-($(ex1), $(ex2)))
	end
	...
	briVec(V::SymVec) = briVec(xComp(V), yComp(V))
	function Base.:+(V1::briVec, V2::briVec)
		x = addExp(V1.x, V2.x)
		y = addExp(V1.y, V2.y)
		briVec(x, y)
	end
	...
	# function fHandle(inputs::Vector{vecInput})
	# 	# :($((input.varName for input in inputs)... ) ->)
	# end

	# function fHandle(inputs::Vector{Symbol})
	# 	# :($((input for input in inputs)... ) -> )
	# end

	function createFun(inputs::Vector{vecInput}, V::briVec)
		lhs = fHandle(inputs)
		rhs = :([$(V.x); $(V.y)])
		:($(lhs) -> $(rhs))
	end
	...
	macro vInputs(exps...)
		return [_vInputs(exp) for exp in exps]
	end

	function _vInputs(exp)
		vecInput((exp))
	end

	
	function applyConstraint(V::Symbol, lh::Symbol, rh::Union{Symbol, Expr, Number})
		if V == lh
			return rh
		end
		return V
	...
	function applyConstraint(V::Expr, lh::Symbol, rh::Union{Symbol, Expr, Number})
		V2 = deepcopy(V)
		for i in 1:length(V2.args)
			V2.args[i] = applyConstraint(V2.args[i], lh, rh)
		end		
		return V2
	end

	
	function applyConstraint(V::briVec, lh::Symbol, rh::Union{Symbol, Expr, Number})
		x = applyConstraint(V.x, lh, rh)
		y = applyConstraint(V.y, lh, rh)
		V2 = briVec(x, y)
		return V2
	end
	

	function applyConstraint(V::Number, lh::Symbol, rh::Union{Symbol, Expr, Number}) 
		return V
	end

	function applyConstraint(V::briVec, D::Dict)
		V2 = deepcopy(V)
		for (key, val) in D
			lh = Symbol(key)
			rh = D[key]
			V2 = applyConstraint(V2, lh, rh)
		end
		return V2
	end
	...
	struct VectorLoop
		...
		Q1::Symbol
		Q2::Symbol
		C::Vector{Union{Expr, Symbol}}
	end
	VectorLoop(I, Q1, Q2) = VectorLoop(I, Q1, Q2, [])
	VectorLoop(I, Q1, Q2, C::Union{Expr, Symbol}) = VectorLoop(I, Q1, Q2, [C])
	VectorLoop(I::vecInput, Q1::vecInput, Q2::vecInput, C) = VectorLoop(I.varName, Q1.varName, Q2.varName, C)

	
	function applyConstraint(V::briVec, vL::VectorLoop)
		V2 = deepcopy(V)
		for constraint in vL.C
			lh = constraint.args[1]
			rh = constraint.args[2]
			V2 = applyConstraint(V2, lh, rh)
		end
		return V2
	end
	...
	function generateFunction(vL::VectorLoop, V::briVec, Input::Number)
		func = deepcopy(V)
		func = applyConstraint(func, vL)
		func = applyConstraint(func, vL.I, Input)
		:(($(vL.Q1), $(vL.Q2)) -> [$(func.x); $(func.y)])
	end

	function generateFunction(vL::VectorLoop, V::briVec)
		func = deepcopy(V)
		func = applyConstraint(func, vL)
...
# ╔═╡ 3fd8d491-72d4-490c-90b2-b44a55d013f2
normalizeRad(X) = mod(X, 2*pi)

# ╔═╡ c597b992-10da-4a7b-be1f-34542423f125
md"""
## How to use package
...
# ╔═╡ 76789db8-f410-45d6-90e1-e7bb6623f304
...
function lineSegment(x, y, V::Vec)
	if V.sign > 0
		x = [x, x+V.R*cos(V.θ)]
		y = [y, y+V.R*sin(V.θ)]
	else
...
function iterativeSolve(fun, initalGuess, tol)
	Xn1 = initalGuess
	J = ForwardDiff.jacobian(fun, Xn1)
	Jinv = inv(J) 
	Xn = -Jinv*fun(Xn1) + Xn1
	diff = abs.(Xn - Xn1)
	Xn1 = Xn
